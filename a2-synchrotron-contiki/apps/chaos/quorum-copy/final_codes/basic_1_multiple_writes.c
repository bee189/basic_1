...
static int complete, tx = 0 , i, txrx_count = 0, done = 0, j = 0, local_round, t, a = 0, flip_coin, intiator_id, set, a1, flag_arry[30], invalid_count=0, retransmitt = 0;
...
// chaos_state_t next_state = CHAOS_RX;
   ...
   chaos_state_t next_state;
	...
	if ((current_state == CHAOS_INIT) &&((quorum_local.phase ==  CAN_READ) || (quorum_local.phase == CAN_WRITE_QUERY) )){
		...
		if(rx_quorum->msg_type == tx_quorum->msg_type ){
		   	...
		   	if (((rx_quorum->seq_nr == tx_quorum->seq_nr) && (rx_quorum->msg_type == READ_HEADER)) 
				   || ((rx_quorum->seq_nr == tx_quorum->seq_nr)&& (rx_quorum->msg_type == WRITE_HEADER)&&(rx_quorum->identity == tx_quorum->identity))){
				...
				/* if( (rx_quorum->msg_type == WRITE_HEADER)){
					if(rx_quorum->identity > tx_quorum->identity){
						tx_quorum->identity = rx_quorum->identity;
						txrx_count = 0;
						tx = 1;		
						}
					else if (rx_quorum->identity < tx_quorum->identity){
						//txrx_count = 0;
						tx = 1;
						}
			...
			if ((tx_quorum->msg_type == WRITE_HEADER)&&(rx_quorum->identity < tx_quorum->identity)){
				tx = 1;
			}
	 
		}
			   
		else if(((rx_quorum->msg_type == WRITE_HEADER) && (tx_quorum->msg_type == READ_HEADER)) && (rx_quorum->seq_nr > tx_quorum->seq_nr)){
			tx_quorum->identity = rx_quorum->identity;
		    txrx_count = 0;					
			memset(tx_quorum->flags, 0, quorum_get_flags_length());
			unsigned int array_index = chaos_node_index / 8;
			...
			/* if((flip_coin == 0) && ((tx_quorum->identity) < node_id)){						
				tx_quorum->identity  = node_id;
				txrx_count = 0;
				tx = 1;						
				} */
		...
		else if((tx_quorum->msg_type == WRITE_HEADER) && (rx_quorum->msg_type == READ_HEADER)){
	...
	if(flip_coin == 0){
	   ...
	   if (a > (CHAOS_NODES/2)){
		   ...
		   if((tx_quorum->msg_type == READ_HEADER) && (rx_quorum->msg_type == READ_HEADER)){
				memset(tx_quorum->flags, 0, quorum_get_flags_length());
				txrx_count = 0;
				done = 0;
				a = 0;
				...
				tx_quorum->seq_nr = tx_quorum->seq_nr+1;
	...
	if(tx){
     ...
     if(((current_state == CHAOS_TX) && (txrx_count > 4))){
		//printf(":Round%u. set bits %u. nodeid %u", round_count, a, tx_quorum->identity);
		//printf("local_seq %u. ", quorum_local.quorum_value.seq_nr);
	...
	if(end) {
		printf(": set bits %u: sender %u: local_seq %u. ", a, tx_quorum->identity, quorum_local.local_seq_nr);
		quorum_local.quorum_value.write_value = tx_quorum->write_value;
        quorum_local.quorum_value.seq_nr = tx_quorum->seq_nr;
		quorum_local.quorum_value.identity = tx_quorum->identity; 
...
flip_coin = rand() %2;
 if(flip_coin == 0){
	quorum_local.phase = CAN_WRITE_QUERY;
	printf("writesel");
