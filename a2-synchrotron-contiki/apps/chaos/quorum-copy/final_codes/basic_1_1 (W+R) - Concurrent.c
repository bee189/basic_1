...
static int complete, tx = 0 , i, txrx_count, done = 0, j = 0, local_round, t, flag_arry[20], a = 0, flip_coin, intiator_id, set;
...
/*  intiator_id = IS_INITIATOR; */
   ...
   //&&  quorum_local.phase != READ_PROPOSE
	...
	if (current_state == CHAOS_INIT && quorum_local.phase != READ_PROPOSE){
     ...
     if(current_state == CHAOS_TX && txrx_count > 3){
